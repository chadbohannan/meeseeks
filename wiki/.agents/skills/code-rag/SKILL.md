...
```bash
python3 ./scripts/code-rag-cli.py status
```

## Workflow

1. **`python3 ./scripts/code-rag-cli.py status`** — confirm the repo is indexed; if missing, stop
2. **`python3 ./scripts/code-rag-cli.py search "<question>"`** — describe what you need in plain English
3. **`python3 ./scripts/code-rag-cli.py fetch <path1> <path2> ...`** — pull full source for the top hits
4. **`python3 ./scripts/code-rag-cli.py browse <repo/dir>`** — explore neighboring code

## Tips

- **Write full sentences**, not keywords. "function that parses CSV uploads" outperforms "csv parse".
- **Use `--glob` to narrow scope**: `python3 ./scripts/code-rag-cli.py search --glob "backend/*.py" "error handling"`.
...
```bash
code-rag-cli.py search "retry logic for HTTP requests"
code-rag-cli.py search --top-k 10 --glob "*.py" "error handling"
```
...
```bash
code-rag-cli.py unit repo/file.py:Class:method
```
...
```bash
code-rag-cli.py fetch repo/a.py:func repo/b.py:Class:method
```
...
```bash
code-rag-cli.py browse
code-rag-cli.py browse myrepo/src
code-rag-cli.py browse myrepo/src/main.py
```
...
```bash
code-rag-cli.py units
code-rag-cli.py units --limit 50 --glob "backend/*.py"
```
...
```bash
code-rag-cli.py files
code-rag-cli.py files --glob "*.ts"
```
...
```bash
code-rag-cli.py clear-repo myrepo
```
