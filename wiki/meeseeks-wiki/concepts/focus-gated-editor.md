...
## Identity-scoped saves

`TicketRoute` is registered at `/workflows/:workflowName/tickets/:filename` without a `key`, so clicking from one ticket to another reuses the same component instance rather than remounting it. Every piece of local state — `title`, `body`, `state`, `color`, `dirty` — therefore survives the switch, and for exactly one render it describes the *outgoing* ticket while the route params, and everything derived from them, already describe the incoming one. React applies the resets queued inside the identity-change effect on the following render, not during this one.

Two refs keep the two clocks apart. `identityRef` is where the route currently points; the identity-change effect advances it. `loadedIdentityRef` is the ticket whose server snapshot actually populated the state variables; only the load effect's adopt branch advances it, and the identity-change effect clears it to `null` on a switch. A `settledIdentity()` helper returns an identity only when both agree, and every save assembled from the render closure resolves its target through it — the debounced body save, `saveIfDirty`, and the project, state, and color handlers. During the one-render window the helper returns `null` and those saves decline to run, which is safe because the outgoing edit has already been written to its own file by then.

This matters most for the read-only lock's effect below, the only save path not driven by a user gesture. Because its `saveIfDirty` dependency gets a new function identity on every render, the effect re-fires on the transition render whenever `locked` is true for the incoming ticket. Before `settledIdentity` existed, that fired `performSave(incoming, { …outgoing fields })`: switching from a ticket with unsaved edits to a ticket whose agent was already running overwrote the running ticket's title, body, and color with the previous one's, and — because `updateTicket` renames the file when the patch carries a `state` — moved it into the previous ticket's lane as well. The symptom read as one ticket's content being replaced by another's.

The flush on the way out is `flushOutgoing(identity)`, which takes its target as an argument instead of reading a ref, and writes the debounced body snapshot merged with the render closure's `title`/`state`/`color` in a single PATCH. Merging matters because the title input has no debounce of its own and relies on a blur that in-route navigation can skip; the pending body snapshot alone would carry a stale title. The unmount cleanup uses the same function, so a dirty title is no longer lost on route exit either.
...
| 2026-08-26 | `src/web/routes/TicketRoute.tsx`, `src/web/App.tsx`, `src/storage/ticket.ts` |
