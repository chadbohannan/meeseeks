...
How Claude Code loads its instructions and context at startup is a distinct concern from how Meeseeks invokes the binary, so it lives on its own page (split out of the [Claude Code](../systems/claude-code.md)). It matters to Meeseeks because the orchestrator generates per-session settings files and may eventually template other `.claude/` contents (rules, commands) at workspace or workflow scope — and because one deliberate divergence, injecting context explicitly rather than relying on auto-discovery, is a design decision worth recording.
...
**`CLAUDE.md`** is the primary instruction file. Claude reads it at the start of every session. It can live in the project root or at `.claude/CLAUDE.md`. Meeseeks does not rely on this auto-discovery for its own context — a [project](project-model.md) carries `context` inline or a `contextFile` pointer, which the [runtime adapter](../components/runtime.md) reads and prepends to the `--append-system-prompt` preamble, ahead of the workflow's `PROCESS.md` and the ticket reference. Explicit injection is intentional: it keeps that context observable in the supervisor's recorded preamble rather than hidden inside Claude Code's startup behavior, and it decouples the source file's name from a tool-specific convention. A `contextFile` commonly *points at* the repository's own `CLAUDE.md`, which is the case the project editor seeds by default — the file is read and injected rather than discovered. Any `CLAUDE.md` files inside the working tree are still picked up by Claude Code's normal discovery as well.
...
Claude Code re-reads instruction and settings files on each tool call. For Meeseeks, this means the workspace's `.claude/` tree — cwd for every spawned agent — can carry tailored instructions that automatically apply to any agent spawned there. Note that a project's context is *not* picked up by this auto-discovery chain: the orchestrator injects it explicitly so its contents are observable in the recorded preamble. This is the instruction-bootstrapping counterpart to the settings-file precedence chain documented in the [sandboxing runbook](../runbooks/claude-code-sandboxing.md).
...
- **Keep global config minimal.** Over-populating `~/.claude/` causes context bleed, where rules from one project interfere with another. For Meeseeks, this argues for putting agent instructions at the workspace or workflow level rather than in the user's global config.
