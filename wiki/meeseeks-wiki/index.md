...
- [Pi](systems/pi.md) — the `pi-mono` coding agent: four invocation modes, an RPC control protocol, Agent Skills, and no separate memory system by design
...
- [Deep Agents](systems/deep-agents.md) — batteries-included coding harness: virtual filesystem, sandboxes, subagents, skills, memory, HITL
- [Deep Agents Code (`dcode`)](systems/deep-agents-code.md) — LangChain's terminal coding agent: durable threads, a lifecycle hook bus, goals and rubrics
- [Managed Deep Agents](systems/managed-deep-agents.md) — hosted CLI-first runtime (`mda`): the runtime/developer ownership split and identity scoping; private beta
...
- [LangChain Models](components/langchain-models.md) — standard multi-provider model interface, structured output strategies, model profiles
...
- [LangSmith Sandboxes](components/langsmith-sandboxes.md) — managed isolated execution: snapshots, the credential-injecting auth proxy, service URLs
