...
The Kanban now supports drag-and-drop between state columns — `TicketCard` is `draggable` and each column registers `onDragOver`/`onDrop` handlers in `Kanban.tsx`, with the active drop target highlighted. The plain-textarea ticket body has been replaced by `MarkdownEditor`, used by ticket bodies, board CLAUDE.md, lane PROCESS.md, skills files, and one-shot prompts. The editor uses a `bodyInitializedRef` focus-stability pattern: once mounted, refetches do not overwrite in-progress edits, so external filesystem changes to an open document only appear on remount. UI tests remain deferred.
