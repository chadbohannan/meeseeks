...
**`CLAUDE.md`** is the primary instruction file. Claude reads it at the start of every session. It can live in the project root or at `.claude/CLAUDE.md`. Meeseeks already uses `--append-system-prompt` to inject ticket context, but `CLAUDE.md` provides persistent, session-spanning instructions that don't need to be re-injected on each spawn.
