...
| Persistence | Present in both: Claude Code `/resume`+`/rewind`, Pi JSONL sessions — but harness-internal, not orchestrator-addressable | Durable by construction; resumes after restart |
...
The two-paradigm frame is a simplification, and the [build-vs-supervise synthesis](harness-sdk-build-vs-supervise.md) supplies the refinement: there is a *third posture* — building an agent on a harness SDK ([Claude Agent SDK](../systems/deep-agents.md) or Deep Agents) rather than supervising a finished CLI or attaching to a running server. That posture straddles the paradigms: you own the agent definition (like the CLI paradigm) but inherit the runtime's durability, streaming, and — with Deep Agents — a shipped Agent Server (like the framework paradigm). It is best understood not as a third paradigm but as the seam through which the supervised-CLI world migrates into the framework/server world. Notably, Deep Agents' own [`dcode` CLI](../systems/deep-agents-code.md) is a supervised-CLI-paradigm surface *over* a framework/server-paradigm runtime — proof that the two are endpoints of a spectrum, not a hard dichotomy.

The July 2026 ingest of the full `dcode` documentation turns that observation from a hedge into the strongest qualification this page carries, because it supplies specifics for four of the table's rows *on the wrong side of the divide*. `dcode` is spawned over stdio like Claude Code and Pi, yet it checkpoints every session to a SQLite database and exposes `--resume` plus a queryable `threads list` (durability without an HTTP client); it emits typed lifecycle events including `input.required` and `permission.request` through a `hooks.json` bus rather than requiring state to be scraped from a byte stream (structural lifecycle state without a server); it accepts `--max-turns`/`--timeout` budgets and a `--recursion-limit` (fault-tolerance controls in a CLI); and it switches providers freely with `--model provider:model`. Four rows that the table attributes to the framework/server paradigm are in fact available *inside* paradigm A, provided the CLI being supervised is itself built on a durable runtime.

What survives intact is the bottom row, and it survives for the right reason. Supervising `dcode` still makes Meeseeks a **process supervisor** — it would still assemble argv, own the child's lifecycle, and render its output. The paradigm attribution in the table was therefore slightly too generous to paradigm B: those capabilities are properties of *the runtime beneath the harness*, not of the orchestrator's relationship to it. Choosing `dcode` would be, in this page's own terms, "choosing a different CLI to supervise" — but unlike choosing Pi, it would import most of the capability list that made paradigm B attractive. That makes it the cheapest available probe of the migration thesis, and it narrows the genuinely paradigm-bound gains to the coordination layer the [Agent Server](../components/langgraph-agent-server.md) provides: double-texting, background runs and rejoin, cron, and managed multi-tenant persistence.
...
That framing is challenged from the product side by the [attention-economics synthesis](attention-economics.md), which accepts that durability and observability are the capabilities worth having but denies they are reasons to migrate. Its argument is that the two separate cleanly from the paradigm: observability is additive to the *current* supervisor via the [LangSmith tracing runbook](../runbooks/tracing-meeseeks-sessions-to-langsmith.md)'s settings-file seam, and durability's product value depends on persisting session state rather than on who owns the agent loop — so a disk-backed ring buffer and runtimes Map would capture much of it without a paradigm change. This page presents durability and persistence as properties *inherited* by moving to paradigm B, which makes them read as arguments for the move; the counter is that they are largely obtainable inside paradigm A, and the genuinely paradigm-bound gains are narrower than the table above implies.
...
| 2026-07-24 | https://docs.langchain.com/oss/python/deepagents/code/cli-reference — durable threads, budgets, and hooks inside a supervised CLI |
