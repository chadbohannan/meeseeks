...
## Subagents are the point where the two economies meet

[Subagents](../concepts/deepagents-delegation.md) are where this page's context-window economy and a third, sibling economy — plain dollar-and-token cost — intersect directly, because dispatching a subagent is simultaneously a context-isolation move (this page's framing) and an instance of running multiple agents (the [swarm-tax synthesis](swarm-tax-multi-agent-cost-benefit.md)'s framing). The swarm-tax page's central finding sharpens the "offload" row in the table above: Anthropic's own multi-agent research system, built from exactly this subagent-isolation pattern, costs roughly 15x a single chat interaction in tokens, and that premium is only justified when the delegated work is read-heavy and independently parallel — the same read/write distinction that determines whether Deep Agents' subagent isolation is cheap architecture or an expensive one. A supervisor dispatching subagents purely to protect its own context window, without regard for whether the delegated work is read-heavy, is paying the swarm tax to solve a problem (context pressure) that offloading or summarization could have solved for a fraction of the cost.
